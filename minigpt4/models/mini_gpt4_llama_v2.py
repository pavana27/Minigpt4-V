...
class MiniGPT4_Video(Blip2Base, PreTrainedModel):
    ...
    def __init__(
        ...
        if self.prompt_path:
            ...
            self.prompt_list = []
    #modifed this function to add the t_images
    def encode_img(self, image):
        ...
        if t_images is not None:
            if len(t_images.shape) > 4:
                t_images = t_images.reshape(-1, *t_images.shape[-3:])
            with self.maybe_autocast():
                t_img_embeds = self.ln_vision(self.visual_encoder(t_images)).to(device)
                t_img_embeds = t_img_embeds[:,1:,:]
                bs, pn, hs = t_img_embeds.shape
                if self.token_pooling:
                    t_img_embeds = t_img_embeds.view(bs, int(pn/4), int(hs*4))
            combined = torch.cat([image_embeds, t_img_embeds], dim=1)
        else:
            combined = image_embeds

        inputs_llama = self.llama_proj(combined)
        atts_llama = torch.ones(inputs_llama.size()[:-1], dtype=torch.long).to(device)
        return inputs_llama, atts_llama
      
    #modifed this function to add the t_images
    def get_context_emb(self, prompt, img_list, t_img_list=None):
        ...
        for i in range(len(img_list)):
            ...
            if t_img_list is not None:
                mixed_embs.append(t_img_list[i])
    ...
    def preparing_embedding(self, samples, t_images=None): 
        ...
        if 'image' in samples:
            img_embeds, img_atts = self.encode_img(samples['image'], t_images)
        else:
    ...
    def forward(self, samples, t_images=None, reduction="mean"):
        # prepare the embedding to condition and the embedding to regress
        cond_embeds, cond_atts, regress_embeds, regress_atts, part_targets = \
            self.preparing_embedding(samples, t_images=t_images)
    ...
    def generate(
        ...
        if img_embeds is None:
            img_embeds, atts_img = self.encode_img(images.to(self.device), t_images.to(self.device) if t_images is not None else None)
        else:
        ...
        if lengths is not None:
            ...
            for idx, img_embed in enumerate(img_embeds):
                ...
                if t_images is not None:
                    t_image_lists.append([img_embed[i][None] for i in range(lengths[idx], lengths[idx]*2)])
                else:
                    t_image_lists.append(None)
        else:
            image_lists = [[image_emb[None]] for image_emb in img_embeds]
            t_image_lists = None

        batch_embs = [self.get_context_emb(text, img_list, t_img_list)
                      for text, img_list, t_img_list in zip(texts, image_lists, t_image_lists)]    
...
#No change needed — it only uses text inputs (seg_tokens)
    @torch.no_grad()
...
#no change is needed- Already works if samples['image'] includes both modalities (handled upstream)
    @torch.no_grad()
...
#no change is needed - Works because it calls self.generate() which is already patched
#Unless we are planning to use t_images specifically in text-only inference or ranking contexts independently.
    def predict_answers(
        self,
        samples,
        num_beams=5,
...
# In assign_imgs(), pass t_img_embeds and interleave
def assign_imgs(batched_instruct_list, batched_img_embeds, batched_t_img_embeds=None):
        if len(batched_img_embeds.shape) == 3:
            batched_img_embeds = batched_img_embeds[:, None]
        if batched_t_img_embeds is not None and len(batched_t_img_embeds.shape) == 3:
            batched_t_img_embeds = batched_t_img_embeds[:, None]

        batched_assigned = []
        for i, instruct_list in enumerate(batched_instruct_list):
            img_idx = 0
            assigned_img = []
            img_embeds = batched_img_embeds[i]
            t_img_embeds = batched_t_img_embeds[i] if batched_t_img_embeds is not None else None
            ...
            for instruct in instruct_list:
                ...
                if n_img > 0:
                    ...
                    for j in range(n_img):
                        imgs.append(img_embeds[img_idx + j])
                        if t_img_embeds is not None:
                            imgs.append(t_img_embeds[img_idx + j])
                    assigned_img.append(torch.stack(imgs))
                    img_idx += n_img
                else:
