...
class EngageNetDataset(BaseDataset, __DisplMixin):
    def __init__(
        ...
        ann_paths,
        subtitles_path,
        t_images_path,  # New parameter for additional images folder
        model_name='llama2',
        add_subtitles=True,
        ...
        vis_root (string): Root directory of images (e.g. coco/images/)
        ann_root (string): directory to store the annotation file
        t_images_path (string): Path to folder containing additional images
        """
        super().__init__(vis_processor, text_processor, vis_root, ann_paths)
        self.model_name=model_name
        if self.model_name =='mistral':
            ...
            self.max_sub_len = 400
        self.add_subtitles = add_subtitles
        self.t_images_path = t_images_path  # Store additional images path
        self.videos_has_subtitles = {}
        if self.add_subtitles:
            self.subtitle_folder = subtitles_path
            for sub in os.listdir(self.subtitle_folder):
    ...
    def __getitem__(self, index):
        ...
        images=[]
        t_images = []  # List to store toeplitz images
        img_placeholder = ""
        has_subtitles = self.videos_has_subtitles.get(video_id, False)
        if self.add_subtitles and has_subtitles:
            subtitle_path = os.path.join(self.subtitle_folder, f'{video_id}.vtt')
            ...
            if frame_count % sampling_interval == 0:
                ...
                if self.add_subtitles and has_subtitles and number_of_sub_words<self.max_sub_len:
                    if subtitle_text_in_interval != "":
                        ...
                        subtitle_text_in_interval = ""
            frame_count += 1
            if len(images) >= 45: #changed this from self.length to 45 (since other 45 is coming from t-images)
                break
        cap.release()
        if len(images) ==0:
            logger.info("Video not found",video_path)
        ...
        # Load additional images from the folder
        t_images_folder = os.path.join(self.t_images_path, video_id)
        if os.path.exists(t_images_folder):
            for img_file in sorted(os.listdir(t_images_folder)):
                img_path = os.path.join(t_images_folder, img_file)
                img = Image.open(img_path).convert("RGB")
                img = self.vis_processor(img)
                t_images.append(img)
        if len(t_images) == 0:
            logger.info("No additional images found for video_id:", video_id)
        t_images = torch.stack(t_images) if t_images else torch.empty(0)

        # Combine the images and the instruction
        instruction = img_placeholder + '\n' + instruction
        # Return the images, additional images, instruction, answer, video_id, and the length of the video
        output = {
            "images": images,
            "t_images": t_images,  # Include additional images
            "answer": answer,
            "image_id": video_id,
            "instruction_input": instruction,
            "length": self.length,
...
class SedDataset(BaseDataset, __DisplMixin):
    def __init__(
        self, 
        vis_processor, 
        text_processor, 
...
class UbfcDataset(BaseDataset, __DisplMixin):
    def __init__(
        self, 
        vis_processor, 
        text_processor, 
